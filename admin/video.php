...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!empty($_FILES['thumbnail']['name']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, ['jpg','jpeg','png','gif','webp']) && $_FILES['thumbnail']['size'] < 5*1024*1024) {
                ...
                if (move_uploaded_file($_FILES['thumbnail']['tmp_name'], $uploadDir . $fname)) {
                    ...
                    if ($old && strpos($old, 'assets/images/video/') === 0) @unlink('../' . $old);
    ...
    elseif ($act === 'delete') {
        ...
        if ($id) {
            $res = $conn->query("SELECT thumbnail FROM video_terkait_db WHERE id=$id");
  ...
  <div class="content-area">
    ...
    <div class="form-card">
      ...
      <form method="POST" enctype="multipart/form-data" id="formVideo">
        ...
        <div class="form-group">
          <label>URL Video (Contoh: https://www.youtube.com/watch?v=xxxxx)</label>
          <input type="url" name="url_video" required placeholder="Masukkan link video..." value="<?= htmlspecialchars($editData['url_video'] ?? '') ?>">
        </div>
