...
function getDB() {
    ...
    // Auto-create tabel berita
    $conn->query("CREATE TABLE IF NOT EXISTS berita (
    ...
    // Seed berita jika tabel kosong
    $cekBerita = $conn->query("SELECT COUNT(*) as cnt FROM berita");
    if ($cekBerita) {
        $row = $cekBerita->fetch_assoc();
        if ((int)$row['cnt'] === 0) {
            $conn->query("INSERT INTO berita (id, judul, kategori, tanggal, gambar, isi) VALUES
            ...
            (2, 'Modernisasi Fasilitas Bengkel Elektronika dengan Peralatan Terkini', 'Teknologi', '2026-06-08', 'assets/images/gedung-bengpus.jpeg', 'Investasi besar dalam peralatan modern untuk mendukung pemeliharaan sistem elektronika pertahanan generasi terbaru.'),
            (3, 'Penandatanganan MoU dengan Industri Pertahanan Australia', 'Kerjasama', '2026-06-01', 'assets/images/sumga.jpeg', 'Kerjasama strategis dengan mitra internasional untuk bertukar ilmu pengetahuan seputar Teknologi.')");
        }
    }

    // Seed pimpinan_db jika tabel kosong
    $cekPimpinan = $conn->query("SELECT COUNT(*) as cnt FROM pimpinan_db");
    if ($cekPimpinan) {
        $row = $cekPimpinan->fetch_assoc();
        if ((int)$row['cnt'] === 0) {
